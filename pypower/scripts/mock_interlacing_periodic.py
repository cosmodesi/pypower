...
plot_poles_fn = os.path.join(base_dir, 'power_window_interlacing_periodic_{}_{}_poles.png')
plot_wedges_fn = os.path.join(base_dir, 'power_window_interlacing_periodic_{}_{}_wedges.png')
...
def plot_poles(sort_by='resampler'):
    ...
    for option1 in options1: # option1 is the value for sort_by, e.g. resampler
        ...
        for ill, ell in enumerate(ells):
            lax[0].plot(kout[mask], kout[mask] * model_conv[ill][mask], linestyle='-', color='k', label='theory * conv' if ill == 0 else None)
