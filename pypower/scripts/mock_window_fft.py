...
bias = 1.5
boxsize = 1000.
boxcenter = np.array([1100., 0., 0.])
nbar = 5e-4
edgesin = np.linspace(1e-5, 0.4, 51)
...
def run_mock(imock=0):
    ...
    power = CatalogFFTPower(data_positions1=data['Position'], data_weights1=data['Weight'], randoms_positions1=randoms['Position'], ells=ells, los=los, edges=edges,
                            boxsize=2000., boxcenter=boxcenter, nmesh=nmesh, resampler='tsc', interlacing=2, position_type='pos')
...
def plot_poles():
    ...
    for ill, ell in enumerate(ells):
        ...
        lax[0].plot(kout, kout*model_conv_wa[ill], linestyle='-', color='C{:d}'.format(ill), label='theory * wa * conv' if ill == 0 else None)
    for ill, ell in enumerate(ells):
